...
public class OrderBook {
  ...
  private void executeTrade(Order restingOrder, Order matchingOrder, MatchContext ctx) {
    ...
    emitOrderUpdate(matchingOrder, ctx);
  ...
  private void emitOrderUpdate(Order order, MatchContext ctx) {
    OrderUpdateEvent update =
            OrderUpdateEvent.builder().sequence(ctx.getSequence()).orderId(order.getOrderId())
  ...
  private void emitTrade(Order restingOrder, Order matchingOrder, Long price, Long quantity,
    ...
    TradeEvent tradeEvent = TradeEvent.builder().sequence(ctx.getSequence()).buyOrderId(buyOrderId)
            .buyClientOrderId(buyClientOrderId).symbol(restingOrder.getSymbol())
            .sellOrderId(sellOrderId).sellClientOrderId(sellClientOrderId).tradePrice(price)
            .quantity(quantity).timestamp(matchingOrder.getTimestamp()).build();
    ctx.emit(tradeEvent);
  }
