 ...
 *
 * <p>
 * This is a ground-up rewrite of SustainedLoadTestRealistic. It was rewritten because the previous
 * harness's OWN bookkeeping — not the engine — was almost certainly the source of the monotonic
 * post-GC heap growth (36M -> 396M) that the benchmarking notes had been chasing as an "unresolved
 * engine leak". Specifically:
 *
 * <ol>
 * <li>{@code restingOrderIds} was an UNBOUNDED ConcurrentLinkedQueue. LIMIT_RATIO (0.50) offered to
 * it, CANCEL_RATIO (0.20) polled from it, netting +30% of every action, forever. At ~76k
 ...
 * RateLimiter — the textbook coordinated-omission bug. Engine stalls hid inside acquire() and never
 * appeared in the tail percentiles.</li>
 ...
 * publisher fills outbound and FAILS the engine, so a heavy listener can trigger the very failure
 * it is trying to measure.</li>
 ...
 *
 * <ul>
 * <li><b>No unbounded harness state.</b> Cancel targets live in a per-thread fixed-size String ring
 * (overwrite-oldest). The one correlation map is self-cleaning (removed on first ack), so its size
 * tracks in-flight and never grows without bound.</li>
 * <li><b>Coordinated-omission-free latency.</b> Latency is measured against each order's INTENDED
 * send time (producerStart + k * interval), not its actual send time. If a producer falls behind
 * because the engine stalled, that delay is fully captured. Pacing (how we approach the target
 * rate) and measurement (intended schedule) are deliberately decoupled.</li>
 * <li><b>Feather-light listener.</b> One map remove + one histogram record, both allocation-free.
 * No per-event object, no queue, no drainer thread. Keeps the publisher fast so it cannot be the
 * thing that trips the engine's outbound-full failure.</li>
 ...
 *
 * <p>
 * Run via {@link #main(String[])} (e.g. exec:java or the IDE). It is intentionally NOT a JUnit test
 * — a saturation run does not belong in the unit suite.
 *
 * <p>
 * DEPTH BALANCE (workload tuning, watch the sampler trace, not a correctness property): the book
...
public final class SustainedLoadHarness {
    ...
    // Ideally gate the measure clock on inbound draining to ~0; a longer warmup is the simpler
    // proxy.
    private static final int WARMUP_SECONDS = 25; // measured-phase JIT/steady-state warmup
    // (discarded)
    private static final int MEASURE_SECONDS = 30; // window that actually feeds the histograms
    private static final String SYMBOL = "AAPL";
    // Tunable via -Dproducers=N. Drop to 2 to free cores for a concurrent collector
    // (ZGC/Shenandoah)
    ...
    static void runAt(long targetRatePerSec) throws Exception {
        ...
        // Approximate count of measured-phase LIMITs currently resting: +1 when a LIMIT is
        // submitted
        ...
        OrderUpdateListener listener = event -> {
            ...
            long latency;
            if (raw < 1) {
                lstats.negative++; // must be ~0 with the strict pacer; a spike means a send
                // preceded
    ...
    private static void produce(MatchingEngine engine, int threadIdx, double intervalNanos,
        ...
        try {
            while (System.nanoTime() < endNanos) {
                ...
                // Implemented by steering the pick value so the existing branch thresholds still
                // hold.
                ...
                try {
                    if (pick < CANCEL_RATIO) {
                        ...
                        if (target == null) {
                            continue; // nothing known-resting yet; skip (counts as neither send nor
                            // fail)
                        }
                        engine.submit(buildCancel(target));
                        ps.cancelSent++;
                        // We do not correlate cancel latency here: the CANCELLED ack carries the
                        // TARGET's clientOrderId (whose intended time is old), and
                        // remove-on-first-ack
                        // has already consumed that id, so it is skipped as non-attributable.
                        // Cancel
                        // *throughput* is captured by cancelSent; cancel *misses* (target already
                        // gone) are an engine-thread rejection with no event, by design invisible
                        // and
                        // not a perf signal.
                        ...
                        boolean buy = rnd.nextBoolean();
                        int level = 1 + rnd.nextInt(LIMIT_CLUSTER_LEVELS); // near the touch,
                        // consumable
                    ...
                    // will be reconciled as in-flight, and submitFailures already flags the
                    // overload.
                    if (terminalState.get() != null) {
                        break;
                    }
    ...
    /**
     * Reflection is confined to reaching private fields on MatchingEngine (books, inboundEvents,
     * disruptor). Depth is then read through OrderBook's PUBLIC snapshot methods, and outbound
     * depth through the Disruptor's RingBuffer. Field names are the coupling point — if they
     * change, this constructor throws with a clear message.
     */
    private static final class EngineReflection {

        private final OrderBook orderBook;
        private final Field inboundField;
        private final Field disruptorField;
        ...
        EngineReflection(MatchingEngine engine, String symbol) {
            ...
            try {
                ...
                // Outbound events now flow through a Disruptor ring buffer, not a plain queue.
                disruptorField = MatchingEngine.class.getDeclaredField("disruptor");
        ...
        int[] queueDepth() {
            // Agrona's concurrent queue implements java.util.Queue, so size() is a plain call and
